...
function IntraDayChartWithStartToToday({ ticker, chartZoomState, setChartZoomState, setCurrentCrossHairX, candleData, timeFrame, chartStartDate, pricePoints, uuid, isZoomAble, currentDiscount, discountPrices, exitAlertPrice })
{
    ...
    const createDateScale = useCallback(({ dateToPixel = undefined, pixelToDate = undefined } = {}) =>
    {
        ...
        let xDateScale = scaleDiscontinuous(scaleTime()).discontinuityProvider(discontinuityRange(...excludedPeriods))
            .domain([subBusinessDays(chartStartDate, 1), addBusinessDays(new Date(), 1)]).range([0, candleDimensions.width])
    ...
    function drawCrossHairs(e)
    {
        ...
        // setCurrentCrossHairX({ svgX: svgCoordinates.x })
...
export default IntraDayChartWithStartToToday