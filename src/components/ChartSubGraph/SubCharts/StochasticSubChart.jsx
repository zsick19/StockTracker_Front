...
import { calculateStochastic, calculateStochastic2, calculateVortex } from '../../../Utilities/technicalIndicatorFunctions'
...
function StochasticSubChart({ candleData, uuid, timeFrame, hideTimeLine })
{
    let periodBlock = 14
    ...
    const vortexData = useMemo(() => calculateStochastic(candleData, 14), [candleData])
    const stochasticData = useMemo(() => calculateStochastic2(candleData), [candleData])
    ...
    const VILine = line().x(d => createDateScale({ dateToPixel: d.date })).y(d => createYScale({ vortexToPixel: d.k })).curve(curveBasis)
    const stochasticLineD = line().x(d => createDateScale({ dateToPixel: d.date })).y(d => createYScale({ vortexToPixel: d.d })).curve(curveBasis)
    ...
    useEffect(() =>
    {
        ...
        vortex.selectAll('.vortexLinePlus').data([stochasticData]).join((enter) => createVortexPlus(enter), update => updateVortexPlus(update))
        ...
        vortex.selectAll('.vortexLineMinus').data([stochasticData]).join((enter) => createVortexMinus(enter), update => updateVortexMinus(update))
        function createVortexMinus(enter)
        {
            enter.append('path').attr('class', 'vortexLineMinus').attr('d', d => stochasticLineD(d)).attr('stroke', 'blue').attr('fill', 'none')
        }
        function updateVortexMinus(update)
        {
            vortex.select('.vortexLineMinus').attr('d', d => stochasticLineD(d))
        }
