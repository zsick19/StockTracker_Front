...
import { addDays, isToday, subMonths, addYears, subDays, startOfMonth, endOfMonth, startOfQuarter, endOfQuarter, eachDayOfInterval, getDay, addHours, addMinutes, differenceInBusinessDays, set, isWeekend, previousFriday, subBusinessDays, addBusinessDays } from 'date-fns'
...
import { generateIntraDayTickMarks, generateIntraDayTickMarksBetweenTwoDates, generateMarketTradingHoursBetweenTwoDates, generateTradingHours, getBreaksBetweenDates, provideStartAndEndDatesForDateScale } from '../../Utilities/TimeFrames'
import { pixelBuffer } from './GraphChartConstants'
import { setYZoomState } from '../../features/Charting/GraphHoverZoomElement'



function BackTestingChart({ ticker, candleData, timeFrame, chartStartDate, chartEndDate, pricePoints, uuid, isZoomAble })
{
    ...
    {
        return eachDayOfInterval({ start: subBusinessDays(chartStartDate, 5), end: addBusinessDays(chartEndDate, 5) })
        // return allDays.filter(day => { const dayOfWeek = getDay(day); return dayOfWeek !== 0 && dayOfWeek !== 6; })
    }, [chartStartDate, chartEndDate])

    const visualBreaksPeriods = useMemo(() => getBreaksBetweenDates(new Date(chartStartDate), new Date(chartEndDate), 'marketOpen'), [chartStartDate, chartEndDate])
    const excludedPeriods = useMemo(() => generateMarketTradingHoursBetweenTwoDates(chartStartDate, chartEndDate), [chartStartDate, chartEndDate])
    const intraDayTickMarks = useMemo(() => generateIntraDayTickMarksBetweenTwoDates(chartStartDate, chartEndDate), [chartStartDate, chartEndDate])
    ...
    const createDateScale = useCallback(({ dateToPixel = undefined, pixelToDate = undefined } = {}) =>
    {
        ...
        let xDateScale = scaleDiscontinuous(scaleTime()).discontinuityProvider(discontinuityRange(...excludedPeriods))
            .domain([subBusinessDays(chartStartDate, 5), addBusinessDays(chartEndDate, 5)]).range([0, candleDimensions.width])
    ...
    }, [candleData, excludedPeriods, chartZoomState?.x, candleDimensions, chartStartDate, chartEndDate])
    ...
    }, [candleDimensions, excludedPeriods, chartZoomState?.x, chartZoomState?.y])
    ...
    useEffect(() =>
    {
        ...
        const xAxis = axisBottom(createDateScale()).tickValues(intraDayTickMarks).tickFormat(timeFormat("%-m/%-d"));
    ...
    }, [candleData, minPrice, maxPrice, excludedPeriods, candleDimensions, chartZoomState?.x, chartZoomState?.y])
    ...
    useEffect(() =>
    {
        ...
        patternSelect.append('rect').attr('class', 'metricVisual')
            ...
            .attr('x', 0).attr('y', entryPricePixel).attr('fill', 'red').attr('opacity', 0.05)
