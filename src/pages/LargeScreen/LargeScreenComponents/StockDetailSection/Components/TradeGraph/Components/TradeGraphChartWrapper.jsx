...
{
    const { data, isSuccess, isLoading, isError, error, refetch } = useGetStockDataUsingTimeFrameQuery({ ...selectedStock, liveFeed: true, info: true })

    let chartContent
    if (isSuccess && data.candleData.length > 0)
    {
        chartContent = <ChartWithChartingWrapper ticker={selectedStock.ticker} candleData={data} chartId={selectedStock.chartId} timeFrame={selectedStock.timeFrame} />
    } else if (isSuccess)
    {
        chartContent = <div>No Data To Display for this ticker</div>
    } else if (isLoading)
    {
