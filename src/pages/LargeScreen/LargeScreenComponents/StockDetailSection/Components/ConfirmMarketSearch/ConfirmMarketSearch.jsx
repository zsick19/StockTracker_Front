...
{
    const { data, isSuccess, isLoading, isError, error, refetch } = useGetUsersUnconfirmedPatternsQuery()
    ...
    async function attemptSubmittingConfirmations()
    {
        try
        {
            const results = await submitConfirmedPatterns({ confirmed: keepTheseTickers.keep, remove: keepTheseTickers.remove }).unwrap()
