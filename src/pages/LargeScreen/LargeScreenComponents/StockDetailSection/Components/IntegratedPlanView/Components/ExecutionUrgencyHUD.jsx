...
{
    const metadata = planData.staticPreCompiledIndicators || {};
    const liveCandles = planData.todaysCandles || [];
    const activeCandle = liveCandles[liveCandles.length - 1] || {};

    const avgWindowMinutes = metadata.averageMinutesInStrikeZone || 15.0;
    const velocityProfile = metadata.executionVelocityRating || "STABLE_ACCUMULATION";

    // Evaluate if real-time 1-minute volume is spiking past your 3-day baseline [INDEX]
    const baselineOneMinVol = planData.dailyTickerValues?.baselineAvgOneMinVolume || 5000;
    ...
    return (
        <div style={{ background: '#111219', padding: '20px', borderRadius: '4px', border: '1px solid #222', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h4 style={{ margin: 0, color: '#00ffff', fontSize: '11px', letterSpacing: '1px' }}>⏱️ TIME-SERIES URGENCY TRACKER</h4>
                {isLiveVolumeClimax && (
                    <span style={{ fontSize: '10px', background: 'rgba(0,255,255,0.1)', color: '#00ffff', padding: '2px 8px', borderRadius: '2px', fontWeight: 'bold', animation: 'pulse 1s infinite' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                ...
                <div style={{ background: '#181922', padding: '12px', borderRadius: '3px', borderLeft: velocityProfile === "HYPER_VELOCITY_SPRING" ? '3px solid #ff5555' : '3px solid #50fa7b' }}>
                    <div style={{ fontSize: '10px', color: '#6272a4', marginBottom: '2px' }}>MEAN ABSORPTION WINDOW</div>
                    <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#fff' }}>{avgWindowMinutes} <span style={{ fontSize: '12px', color: '#6272a4' }}>MIN</span></div>
                    <div style={{ fontSize: '9px', color: '#888', marginTop: '4px' }}>REGIME: {velocityProfile.replace('_', ' ')}</div>
                </div>
                ...
                <div style={{ background: '#181922', padding: '12px', borderRadius: '3px', borderLeft: isOpeningCrossActive ? '3px solid #ffb86c' : '3px solid #333' }}>
                    <div style={{ fontSize: '10px', color: '#6272a4', marginBottom: '2px' }}>SESSION TIMING PROFILE</div>
                    <div style={{ fontSize: '15px', fontWeight: 'bold', color: isOpeningCrossActive ? '#ffb86c' : '#fff', marginTop: '4px' }}>
                        {isOpeningCrossActive ? "📈 OPENING AUCTION CROSS" : "💤 MIDDAY CHURN CORRIDOR"}
                    </div>
